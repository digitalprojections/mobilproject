...
public interface ChapterTitleDAO {
   ...
   //Query("SELECT * FROM quran_text WHERE language_id LIKE 1 AND sura_id = :surah_id ORDER BY verse_id ASC")
    ...
    @Query("SELECT q.id, q.sura_id, q.language_id, q.order_no, q.read_count, q.surah_type, q.share_count, q.audio_progress, q.favourite, q.verse_id, q.ayah_text as ar_text, " +
            "(SELECT qe.ayah_text FROM quran_text qe WHERE qe.sura_id LIKE :surah_id AND qe.verse_id = q.verse_id AND qe.language_id = 59) as en_text, " +
            "(SELECT qr.ayah_text FROM quran_text qr WHERE qr.sura_id LIKE :surah_id AND qr.verse_id = q.verse_id AND qr.language_id = 79) as ru_text, " +
            "(SELECT qu.ayah_text FROM quran_text qu WHERE qu.sura_id LIKE :surah_id AND qu.verse_id = q.verse_id AND qu.language_id = 120) as uz_text " +
            "FROM quran_text q WHERE q.sura_id LIKE :surah_id AND q.language_id LIKE 1 ORDER BY q.verse_id ASC")
    LiveData<List<AllTranslations>> getChapterText( String surah_id);

    @Query("SELECT q.id, q.sura_id, q.verse_id, q.favourite, q.order_no, q.read_count, q.surah_type, q.share_count, q.audio_progress, " +
            "qe.ayah_text as en_text, qr.ayah_text as ru_text, qu.ayah_text as uz_text,  q.ayah_text as ar_text " +
            "FROM quran_text q JOIN quran_text qe ON qe.sura_id = q.sura_id " +
            "AND qe.verse_id = q.verse_id JOIN quran_text qr ON qr.sura_id = q.sura_id " +
            "AND qr.verse_id = q.verse_id JOIN quran_text qu ON qu.sura_id=q.sura_id " +
            "AND qu.verse_id = q.verse_id " +
            "WHERE q.favourite LIKE 1 " +
            "AND q.language_id = 1 " +
            "AND qr.language_id = 79 " +
            "AND qe.language_id = 59 " +
            "AND qu.language_id = 120 " +
            "ORDER BY q.sura_id, q.verse_id")
   ...
   @Query("SELECT id, verse_id, sura_id, language_id, share_count, ayah_text as ar_text, audio_progress " +
           "FROM quran_text WHERE language_id=1 AND sura_id =:suraid AND verse_id BETWEEN :start AND :end ORDER BY verse_id ASC")
    LiveData<List<AyahRange>> getAyahRange(String suraid, String start, String end);

    @Query("UPDATE quran_text SET audio_progress=:audioProgress " +
            "WHERE language_id=1 AND sura_id=:suraNumber AND verse_id=:ayahNumber")
    void update(int suraNumber, int ayahNumber, int audioProgress);


    //AND ayah_text LIKE '%' || :word  || '%'
    @Query("SELECT q.id, q.sura_id, q.verse_id, q.favourite, q.order_no, q.read_count, q.surah_type, q.share_count, q.audio_progress, " +
            "qe.ayah_text as en_text, qr.ayah_text as ru_text, qu.ayah_text as uz_text,  q.ayah_text as ar_text " +
            "FROM quran_text q JOIN quran_text qe ON qe.sura_id = q.sura_id " +
            "AND qe.verse_id = q.verse_id JOIN quran_text qr ON qr.sura_id = q.sura_id " +
